...
void SerialSolve(double*A, double*b, int n, int k, int w);
...
void SerialSolve(double *A, double *b, int n, int k, int w){
  ...
  for(i=0; i<n-1; i++){
    for(j=i+1; j<half_k+i+1 && j<n; j++){
      A[j*k + half_k - 1 - (j-i-1)] /= A[i*k + half_k];
    }
    for(j=i+1; j<half_k+i+1 && j<n; j++){
      for(l=i+1; l<half_k+i+1 && l<n; l++){
        A[j*k + half_k + (l-i-1) - (j-i-1)] -= A[j*k + half_k - 1 - (j-i-1)]*A[i*k + half_k + 1 + (l-i-1)];
      }
    }
  } 
  ...
  for(l=0; l<w; l++){
    for(i=0; i<n; i++){
      for(j=i+1; j<half_k+i+1 && j<n; j++){
        b[j+n*l] -= b[i+n*l]*A[j*k + half_k - 1 - (j-i-1)];
      }
    }
    ...
    for(i=n-1; i>=0; i--){
      ...
      for(j=i-1; j>i-1-half_k && j>=0; j--){
        b[j+n*l] -= b[i+n*l]*A[j*k + half_k + 1 + (i-1 - j)];
      }
    }
  }
